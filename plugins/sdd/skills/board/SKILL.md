...
authors, claims, edits, or merges. It runs on **any model tier** (no frontier model needed). The
...
- `.beads/` absent in this project → there's no backlog yet. Tell the user to author a story with
  `/specify <description>` first. Stop.
...
- **Solver** = the story's recommended tier (`budget`/`medium`/`frontier`) read from its `solver-*`
  label — the cost-effective call `/specify` made at authoring time. Blank if unset (a story authored
  before this existed).
...
- Close with the next actions: `/solve <id>` to work a READY story, `/validate <id>` to review a
...
`/solve <id>` if READY, `/validate <id>` if DONE, `/refine <id>` if it carries `needs-refinement`.
...
Read-only discipline: `/board` never writes to bd. Authoring is `/specify` (new) and `/refine`
(revise); solving is `/solve`; merging is `/validate`.
